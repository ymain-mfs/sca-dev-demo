define('Widget.Model', ['SC.Model', 'Application'], function (SCModel, Application)
{
    ...
    ,   getWidgetShapes: function()
        {
            console.log('Widget model', 'getWidgetShapes');
            var recWidget = nlapiCreateRecord('customrecord_gen_widget');
            var fldShape = recWidget.getField('custrecord_gen_widget_shape');
            var widgetOptions = fldShape.getSelectOptions();
            
            var widgetShapes = [];
            for (var i=0; i < widgetOptions.length; i++){
                widgetShapes.push({
                    internalid: widgetOptions[i].getId()
                ,   name: widgetOptions[i].getText()
                });
            }

            console.log('Widget shapes', JSON.stringify(widgetShapes));
